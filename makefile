...
AWFMINDEX_STATIC_LIB_OUTPUT_DIR	=	$(AWFMINDEX_PROJECT_DIR)/bin
...
#static libs
AWFMINDEX_STATIC_LIB_FILE					= $(AWFMINDEX_BUILD_DIR)/$(AWFMINDEX_STATIC_LIB_FILENAME)
LIBDIVSUFSORT_STATIC_LIB_FILE			= $(AWFMINDEX_BUILD_DIR)/$(LIBDIVSUFSORT_STATIC_LIB_FILENAME)
AWFMINDEX_STATIC_LIB_BUILD_SRC		= $(AWFMINDEX_BUILD_DIR)/libawfmindex.a
AWFMINDEX_STATIC_LIB_DEST_SRC			= $(AWFMINDEX_STATIC_LIB_OUTPUT_DIR)/libawfmindex.a
LIBDIVSUFSORT_STATIC_LIB_BUILD_SRC = $(LIBDIVSUFSORT_BUILD_DIR)/lib/libdivsufsort64.a
LIBDIVSUFSORT_STATIC_LIB_DEST_SRC	= $(AWFMINDEX_STATIC_LIB_OUTPUT_DIR)/libdivsufsort64.a
...
.PHONY: static
static: $(AWFMINDEX_BUILD_LIBRARY_DIR) $(OBJECT_FILES) $(LIBDIVSUFSORT_STATIC_LIB_BUILD_SRC)
	ar rcs $(AWFMINDEX_STATIC_LIB_FILE) $(OBJECT_FILES)
	cp $(LIBDIVSUFSORT_STATIC_LIB_BUILD_SRC) $(LIBDIVSUFSORT_STATIC_LIB_DEST_SRC)
	cp $(AWFMINDEX_STATIC_LIB_BUILD_SRC) $(AWFMINDEX_STATIC_LIB_DEST_SRC)
...
clean:
	...
	rm -f $(LIBDIVSUFSORT_STATIC_LIB_DEST_SRC)
	rm -f $(AWFMINDEX_STATIC_LIB_DEST_SRC)
...
$(LIBDIVSUFSORT_BUILD_HEADER_FILE): $(LIBDIVSUFSORT_BUILD_DIR)
	cd $(LIBDIVSUFSORT_BUILD_DIR) &&	cmake -DCMAKE_BUILD_TYPE="Release" -DBUILD_DIVSUFSORT64:BOOL=ON  -DCMAKE_INSTALL_PREFIX="$(PREFIX)" .. && make
...
$(AWFMINDEX_STATIC_LIB_OUTPUT_DIR):
	mkdir -p $(AWFMINDEX_STATIC_LIB_OUTPUT_DIR)

$(LIBDIVSUFSORT_STATIC_LIB_BUILD_SRC):
	cd $(LIBDIVSUFSORT_BUILD_DIR) &&	cmake -DCMAKE_BUILD_TYPE="Release" -DUSE_OPENMP="ON" -DBUILD_SHARED_LIBS="OFF" -DBUILD_DIVSUFSORT64:BOOL=ON  -DCMAKE_INSTALL_PREFIX="$(PREFIX)" .. && make
