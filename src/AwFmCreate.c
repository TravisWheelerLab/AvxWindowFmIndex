...
void populateKmerSeedTableRecursive(struct AwFmIndex *restrict const index,
                                    struct AwFmSearchRange range, uint8_t currentKmerLength,
                                    uint64_t currentKmerIndex, uint64_t letterIndexMultiplier);

void compressSuffixArrayInPlace(uint64_t *const suffixArray, uint64_t suffixArrayLength,
                                const uint8_t compressionRatio);

void fullSequenceSanitize(const uint8_t *restrict const sequence,
                          uint8_t *restrict const sanitizedSequenceCopy,
...
enum AwFmReturnCode awFmCreateIndex(struct AwFmIndex *restrict *index,
    ...
    // set the index out arg initally to NULL, if this function fully completes this will get
    // overwritten
    *index = NULL;
    ...
    // create the suffix array, storing it starting in the second element of the suffix array we
    // allocated. this doesn't clobber the sentinel we added earlier, and makes for easier bwt
    // creation.
    int64_t divSufSortReturnCode =
        divsufsort64(sanitizedSequenceCopy, (int64_t *)(suffixArray), suffixArrayLength);
    ...
    // create the file
    enum AwFmReturnCode returnCode = awFmWriteIndexToFile(
        indexData, suffixArray, sequence, sequenceLength, fileSrc, allowFileOverwrite);
    ...
    if (metadata->keepSuffixArrayInMemory) {
        ...
        if (metadata->suffixArrayCompressionRatio != 1) {
            compressSuffixArrayInPlace(suffixArray, suffixArrayLength,
                                       indexData->metadata.suffixArrayCompressionRatio);
            uint64_t *compressedSuffixArray = realloc(
                suffixArray, awFmGetCompressedSuffixArrayLength(indexData) * sizeof(uint64_t));
        ...
        // if the suffix array isn't supposed to be kept in memory, there's no need to keep it, so
        // it gets freed here.
        indexData->inMemorySuffixArray = NULL;
        free(suffixArray);
    }
...
void setBwtAndPrefixSums(struct AwFmIndex *restrict const index, const size_t bwtLength,
    ...
    if (index->metadata.alphabetType == AwFmAlphabetNucleotide) {
        ...
        for (uint64_t suffixArrayPosition = 0; suffixArrayPosition < bwtLength;
            ...
            uint8_t *restrict const letterBitVectorBytes =
                (uint8_t *)nucleotideBlockPtr->letterBitVectors;

            if (__builtin_expect(positionInBlock == 0, 0)) {
                // when we start a new block, copy over the base occurrences, and initialize the bit
                // vectors while we only use 5 elements, copy over all 8 (to preserve padding and so
                // valgrind doesn't complain about invalid writes)
                memcpy(nucleotideBlockPtr->baseOccurrences, baseOccurrences, 8 * sizeof(uint64_t));
                memset(nucleotideBlockPtr->letterBitVectors, 0,
                       sizeof(__m256i) * AW_FM_NUCLEOTIDE_VECTORS_PER_WINDOW);
            }
            ...
            if (__builtin_expect(sequencePositionInSuffixArray == 0, 0)) {
                // set to the sentinel value (index 5) if we're looking at the letter before the
                // first character.
                letterIndex = 5;
            } else {
            ...
            letterBitVectorBytes[byteInVector] |= ((letterAsCompressedVector >> 0) & 0x1)
                                                  << bitInVectorByte;
            letterBitVectorBytes[byteInVector + 32] |= ((letterAsCompressedVector >> 1) & 0x1)
                                                       << bitInVectorByte;
            letterBitVectorBytes[byteInVector + 64] |= ((letterAsCompressedVector >> 2) & 0x1)
                                                       << bitInVectorByte;
        }
        ...
        for (uint64_t suffixArrayPosition = 0; suffixArrayPosition < bwtLength;
            ...
            const uint8_t byteInVector = positionInBlock / 8;
            const uint8_t bitInVectorByte = positionInBlock % 8;
            struct AwFmAminoBlock *restrict const aminoBlockPointer =
            ...
            if (__builtin_expect(positionInBlock == 0, 0)) {
                // when we start a new block, copy over the base occurrences, and initialize the bit
                // vectors while we only use 21 elements, copy over all 24 (to preserve padding and
                // so valgrind doesn't complain about invalid writes)
                memcpy(aminoBlockPointer->baseOccurrences, baseOccurrences, 24 * sizeof(uint64_t));
                memset(aminoBlockPointer->letterBitVectors, 0,
                       sizeof(__m256i) * AW_FM_AMINO_VECTORS_PER_WINDOW);
            }
            ...
            if (__builtin_expect(sequencePositionInSuffixArray != 0, 1)) {
                ...
                // set to the sentinel value (index 21) if we're looking at the letter before the
                // first character.
                letterIndex = 21;
            }
...
void populateKmerSeedTableRecursive(struct AwFmIndex *restrict const index,
    ...
    for (uint8_t extendedLetter = 0; extendedLetter < alphabetSize; extendedLetter++) {
        ...
        populateKmerSeedTableRecursive(index, newRange, currentKmerLength + 1, newKmerIndex,
...
inline void fullSequenceSanitize(const uint8_t *restrict const sequence,
                                 uint8_t *restrict const sanitizedSequenceCopy,
