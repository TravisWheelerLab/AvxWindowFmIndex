...
enum AwFmReturnCode awFmWriteIndexToFile(struct AwFmIndex *restrict const index, const uint8_t *restrict const sequence,
	...
	size_t elementsWritten =
			fwrite(IndexFileFormatIdHeader, sizeof(char), IndexFileFormatIdHeaderLength, index->fileHandle);
	...
	elementsWritten =
			fwrite(index->kmerSeedTable, sizeof(struct AwFmSearchRange), numElementsInKmerSeedTable, index->fileHandle);
	...
	elementsWritten =
			fwrite(index->suffixArray.values, sizeof(uint8_t), downsampledSuffixArrayLengthInBytes, index->fileHandle);
	...
	if(indexContainsFastaVector) {
		...
		size_t headerStringBytesWritten =
				fwrite(index->fastaVector->header.charData, sizeof(char), headerStringLength, index->fileHandle);
		if(headerStringBytesWritten != headerStringLength) {
			fclose(index->fileHandle);
			return AwFmFileWriteFail;
		}
		size_t fastaVectorMetadataElementsWritten = fwrite(index->fastaVector->metadata.data,
				sizeof(struct FastaVectorMetadata), fastaVectorMetadataLength, index->fileHandle);
...
enum AwFmReturnCode awFmReadIndexFromFile(
	...
	size_t compressedSuffixArrayByteLength =
			awFmComputeCompressedSaSizeInBytes(bwtLength, config.suffixArrayCompressionRatio);
...
enum AwFmReturnCode awFmGetSuffixArrayValueFromFile(
	...
	uint8_t valueBuffer[9]						 = {
			0};	 // 9 bytes are required for this buffer, since ceil(((bitOffset=7) + (valBitWidth=64)) / 8) = 9
...
size_t awFmGetSequenceFileOffset(const struct AwFmIndex *restrict const index) {
	...
	return IndexFileFormatIdHeaderLength + configLength + bwtLengthDataLength + bwtLengthInBytes +
