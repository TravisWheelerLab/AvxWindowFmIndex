...
enum AwFmReturnCode awFmWriteIndexToFile(struct AwFmIndex *restrict const index,
    ...
    // write the header
    size_t elementsWritten = fwrite(IndexFileFormatIdHeader, sizeof(char),
    ...
    elementsWritten =
        fwrite(&index->metadata.versionNumber, sizeof(uint16_t), 1, index->fileHandle);
    if (elementsWritten != 1) {
        fclose(index->fileHandle);
        return AwFmFileWriteFail;
    }
    elementsWritten =
        fwrite(&index->metadata.suffixArrayCompressionRatio, sizeof(uint8_t), 1, index->fileHandle);
    if (elementsWritten != 1) {
        fclose(index->fileHandle);
        return AwFmFileWriteFail;
    }
    elementsWritten =
        fwrite(&index->metadata.kmerLengthInSeedTable, sizeof(uint8_t), 1, index->fileHandle);
    if (elementsWritten != 1) {
        fclose(index->fileHandle);
        return AwFmFileWriteFail;
    }
    ...
    elementsWritten = fwrite(index->bwtBlockList.asNucleotide, bytesPerBwtBlock, numBlockInBwt,
                             index->fileHandle);
    ...
    elementsWritten =
        fwrite(index->prefixSums, sizeof(uint64_t), prefixSumsLength, index->fileHandle);
    ...
    for (size_t i = 0; i < index->bwtLength; i += index->metadata.suffixArrayCompressionRatio) {
        // write the 'implicit' position of the sentinel character at the end in the first element
        // of the SA
...
enum AwFmReturnCode awFmReadIndexFromFile(struct AwFmIndex *restrict *restrict index,
    ...
    // create a local-scope pointer for the index, when we're done we'll set the index out-arg to
    // this pointer.
    ...
    size_t elementsRead =
        fread(headerBuffer, sizeof(char), IndexFileFormatIdHeaderLength, fileHandle);
    ...
    const size_t bytesPerBwtBlock = indexData->metadata.alphabetType == AwFmAlphabetNucleotide
                                        ...
                                        : sizeof(struct AwFmAminoBlock);
    elementsRead =
        fread(indexData->bwtBlockList.asNucleotide, bytesPerBwtBlock, numBlockInBwt, fileHandle);
    ...
    if (keepSuffixArrayInMemory) {
        ...
        if (indexData->inMemorySuffixArray == NULL) {
            ...
            elementsRead = fread(indexData->inMemorySuffixArray, sizeof(uint64_t),
                                 compressedSuffixArrayLength, fileHandle);
    ...
    if (index->metadata.keepSuffixArrayInMemory) {
        for (size_t i = 0; i < positionArrayLength; i++) {
            size_t indexInSuffixArray =
                positionArray[i] / index->metadata.suffixArrayCompressionRatio;
        ...
        for (size_t i = 0; i < positionArrayLength; i++) {
            ...
            uint64_t seekPosition =
                index->suffixArrayFileOffset + (indexInSuffixArray * sizeof(uint64_t));

            size_t bytesRead =
                pread(index->fileDescriptor, &(positionArray[i]), sizeof(uint64_t), seekPosition);
...
enum AwFmReturnCode awFmReadSequenceFromFile(const struct AwFmIndex *restrict const index,
    ...
    const size_t actualPriorFlankLength =
        (bufferWouldUnderflowSequence ? sequencePosition : priorFlankLength);

    const size_t seekPosition =
        index->sequenceFileOffset + sequencePosition - actualPriorFlankLength;
    const size_t sequenceSegmentLength = actualPriorFlankLength + postFlankLength;
    size_t bytesRead = pread(index->fileDescriptor, sequenceBuffer,
...
awFmSuffixArrayReadPositionParallel(const struct AwFmIndex *restrict const index,
    ...
    if (index->metadata.keepSuffixArrayInMemory) {
        uint64_t suffixArrayPosition =
            backtracePtr->position / index->metadata.suffixArrayCompressionRatio;

        backtracePtr->position =
            index->inMemorySuffixArray[suffixArrayPosition] + backtracePtr->_offset;
        backtracePtr->position %= index->bwtLength; // handles the edge case of wrapping around the
                                                    // end of the suffix array.
        ...
        uint64_t suffixArrayPosition =
            backtracePtr->position / index->metadata.suffixArrayCompressionRatio;
        const size_t suffixArrayFileOffset =
            index->suffixArrayFileOffset + (suffixArrayPosition * sizeof(uint64_t));

        ssize_t numBytesRead = pread(index->fileDescriptor, &suffixArrayPosition, sizeof(uint64_t),
                                     suffixArrayFileOffset);
        if (numBytesRead == sizeof(uint64_t)) {
            backtracePtr->position = suffixArrayPosition + backtracePtr->_offset;
            backtracePtr->position %= index->bwtLength; // handles the edge case of wrapping around
                                                        // the end of the suffix array.
            return AwFmFileReadOkay;
        } else {
...
size_t awFmGetSequenceFileOffset(const struct AwFmIndex *restrict const index) {
    ...
    const size_t prefixSumLengthInBytes =
        awFmGetPrefixSumsLength(index->metadata.alphabetType) * sizeof(uint64_t);
    const size_t kmerSeedTableLength = awFmGetKmerTableLength(index);

    return IndexFileFormatIdHeaderLength + metadataLength + bwtLengthDataLength + bwtLengthInBytes +
