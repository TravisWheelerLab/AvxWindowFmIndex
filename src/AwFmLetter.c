...
uint8_t awFmAsciiAminoAcidToLetterIndex(const uint8_t asciiLetter) {
    if (__builtin_expect(asciiLetter == '$', 0)) {
        ...
        static const uint8_t letterEncodings[32] = {20, 0,  20, 1,  2,  3,  4,  5,  6,  7,  20,
                                                    8,  9,  10, 11, 20, 12, 13, 14, 15, 16, 20,
                                                    17, 18, 20, 19, 20, 20, 20, 20, 20, 20};
...
uint8_t awFmAsciiAminoLetterSanitize(const uint8_t asciiLetter) {
    ...
    bool letterIsAmbiguityChar = letterAsLowerCase == 'b' || letterAsLowerCase == 'x';
...
uint8_t awFmAminoAcidCompressedVectorToLetterIndex(const uint8_t compressedVectorLetter) {
    static const uint8_t letterLookup[32] = {21, 18, 19, 2,  13, 16, 3,  20, 11, 12, 15,
                                             20, 0,  20, 20, 20, 20, 20, 20, 14, 20, 8,
                                             17, 1,  20, 7,  5,  6,  9,  10, 4,  20};
