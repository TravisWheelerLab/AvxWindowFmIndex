...
uint8_t awFmAsciiAminoLetterSanitize(const uint8_t asciiLetter) {
	...
	bool letterIsAmbiguityChar = letterAsLowerCase == 'b' || letterAsLowerCase == 'x' || asciiLetter == '\0';
...
uint8_t awFmAminoAcidLetterIndexToCompressedVector(const uint8_t letterIndex) {
	static const uint8_t compressedVectorLookup[22] = {0x0C, 0x17, 0x03, 0x06, 0x1E, 0x1A, 0x1B, 0x19, 0x15, 0x1C, 0x1D,
...
uint8_t awFmAminoAcidCompressedVectorToLetterIndex(const uint8_t compressedVectorLetter) {
	static const uint8_t letterLookup[32] = {21, 18, 19, 2, 13, 16, 3, 20, 11, 12, 15, 20, 0, 20, 20, 20, 20, 20, 20, 14,
