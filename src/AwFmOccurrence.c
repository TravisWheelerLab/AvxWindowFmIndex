...
__m256i
awFmMakeNucleotideOccurrenceVector(const struct AwFmNucleotideBlock *restrict const blockPtr,
    ...
    case 4: // ambiguity character ? 0b010
...
__m256i awFmMakeAminoAcidOccurrenceVector(const struct AwFmAminoBlock *restrict const blockPtr,
    ...
    case 1: /*C (cysteine) encoding 0b10111*/
        return _mm256_andnot_si256(bit3Vector,
                                   _mm256_and_si256(_mm256_and_si256(bit4Vector, bit2Vector),
                                                    _mm256_and_si256(bit1Vector, bit0Vector)));
    ...
    case 4: /*F (Phenylalanine) encoding 0b11110*/
        return _mm256_andnot_si256(bit0Vector,
                                   _mm256_and_si256(_mm256_and_si256(bit4Vector, bit3Vector),
                                                    _mm256_and_si256(bit2Vector, bit1Vector)));
    ...
    case 6: /*H (Histidine) encoding 0b11011*/
        return _mm256_andnot_si256(bit2Vector,
                                   _mm256_and_si256(_mm256_and_si256(bit4Vector, bit3Vector),
                                                    _mm256_and_si256(bit1Vector, bit0Vector)));
    ...
    case 10: /*M (Methionine) encoding 0b11101*/
        return _mm256_andnot_si256(bit1Vector,
                                   _mm256_and_si256(_mm256_and_si256(bit4Vector, bit3Vector),
                                                    _mm256_and_si256(bit2Vector, bit0Vector)));
    case 11: /*N (Asparagine) encoding 0b01000*/
        return _mm256_andnot_si256(_mm256_or_si256(bit0Vector, bit1Vector),
                                   _mm256_andnot_si256(bit2Vector, bit3Vector));
    ...
    case 13: /*Q (glutamine) encoding 0b00100*/
        return _mm256_andnot_si256(_mm256_or_si256(bit3Vector, bit1Vector),
                                   _mm256_andnot_si256(bit0Vector, bit2Vector));
    ...
    case 18: /*W (Tryptophan) encoding 0b00001*/
        return _mm256_andnot_si256(_mm256_or_si256(bit3Vector, bit2Vector),
                                   _mm256_andnot_si256(bit1Vector, bit0Vector));
    case 19: /*Y (Tyrosine) encoding 0b00010*/
        return _mm256_andnot_si256(_mm256_or_si256(bit0Vector, bit2Vector),
                                   _mm256_andnot_si256(bit3Vector, bit1Vector));
    case 20: /*ambiguity character Z 0b11111 */
        return _mm256_and_si256(
            _mm256_and_si256(bit4Vector, bit3Vector),
            _mm256_and_si256(bit2Vector, _mm256_and_si256(bit1Vector, bit0Vector)));
    // 0b00000 is sentinel, but since you can't search for sentinels, it is not included here.
    default:
        __builtin_unreachable(); // GCC respects this, doesn't check for letters that aren't valid
    }
}
...
inline uint16_t awFmVectorPopcount(const __m256i occurrenceVector,
    ...
    // deprecated
    // const __m256i lowBitsLookupTable = _mm256_set_epi8( 4, 3, 3, 2, 3, 2, 2, 1, 3, 2, 2, 1, 2, 1,
    // 1, 0, 4, 3, 3, 2, 3, 2, 2, 1, 3, 2, 2, 1, 2, 1, 1, 0); const __m256i highBitsLookupTable =
    // _mm256_set_epi8(-5,-4,-4,-3,-4,-3,-3,-2,-4,-3,-3,-2,-3,-2,-2,
    // -1,-5,-4,-4,-3,-4,-3,-3,-2,-4,-3,-3,-2,-3,-2,-2, -1); const __m256i lowNybbleBitmasked =
    // _mm256_and_si256(occurrenceVector, _mm256_set1_epi8(0x0F)); const __m256i lowNybbleBitCount =
    // _mm256_shuffle_epi8(lowBitsLookupTable, lowNybbleBitmasked); const __m256i highNybbleBits =
    // _mm256_srli_epi16(occurrenceVector, 4); const __m256i highNybbleBitmasked =
    // _mm256_and_si256(highNybbleBits, _mm256_set1_epi8(0x0F)); const __m256i
    // highNybbleNegativeBitCount = _mm256_shuffle_epi8(highBitsLookupTable, highNybbleBitmasked);
    // const __m256i sadCountVector = _mm256_sad_epu8(lowNybbleBitCount,
    // highNybbleNegativeBitCount);
    //
    ...
    // //224 = 256 - 32, where the 32 is from subtracting one extra from each in the
    // highBitsLookupTable
    // // keeping all the high bitmask one less than actual forces every add to overflow, making
    // this consistent.
    //
    // return finalSum;
}
...
inline void awFmBlockPrefetch(const void *restrict const baseBlockListPtr,
                              const uint64_t blockByteWidth, const uint64_t nextQueryPosition) {

    const uint64_t blockIndex = awFmGetBlockIndexFromGlobalPosition(nextQueryPosition);
    // make the blockAddress pointer as a uint8_t* to make clean and easy pointer arithmetic when
    // defining cache line boundries.
...
//     const uint8_t *restrict const blockVectorAsByteArray = (uint8_t*)(letterBitVectorPtr
//     +letterBit); const uint8_t bit = (blockVectorAsByteArray[byteInBlock] >> bitInBlockByte) & 1;
//
//     letter = (letter << 1) | bit;
//   }
//
 ...
 *    bwtPosition: Position of the character to be returned, relative to the start of the block to
 * query.
 *
 *  Returns:
 *    Letter index of the character at the bwtPosition in the specified blockList.
 */
uint8_t awFmGetNucleotideLetterAtBwtPosition(const struct AwFmNucleotideBlock *blockPtr,
                                             const uint8_t localPosition) {
    const uint8_t byteInBlock = localPosition / 8;
    const uint8_t bitInBlockByte = localPosition % 8;

    const uint8_t *restrict const letterBytePointer =
