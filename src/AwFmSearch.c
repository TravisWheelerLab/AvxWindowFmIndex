...
void awFmNucleotideIterativeStepBackwardSearch(const struct AwFmIndex *restrict const index,
    ...
    uint8_t *newStartBlockPtr = ((uint8_t *)index->bwtBlockList.asNucleotide) +
...
void awFmAminoIterativeStepBackwardSearch(const struct AwFmIndex *restrict const index,
    ...
    uint8_t *newEndBlockPtr =
        ((uint8_t *)index->bwtBlockList.asAmino) + (newEndBlock * sizeof(struct AwFmAminoBlock));
    for (size_t cacheLine = 0; cacheLine < 5; cacheLine++) {
...
inline size_t awFmNucleotideBacktraceBwtPosition(const struct AwFmIndex *restrict const index,
    ...
    const struct AwFmNucleotideBlock *restrict const blockPtr =
...
inline size_t awFmAminoBacktraceBwtPosition(const struct AwFmIndex *restrict const index,
                                            const uint64_t bwtPosition) {
    const uint64_t *prefixSums = index->prefixSums;
    const uint64_t blockIndex = awFmGetBlockIndexFromGlobalPosition(bwtPosition);
    const uint8_t localQueryPosition = awFmGetBlockQueryPositionFromGlobalPosition(bwtPosition);
...
inline void awFmNucleotideNonSeededSearch(const struct AwFmIndex *restrict const index,
    ...
    while (indexInKmerString-- != 0 && awFmSearchRangeIsValid(range)) {
        awFmNucleotideIterativeStepBackwardSearch(
            index, range, awFmAsciiNucleotideToLetterIndex(kmer[indexInKmerString]));
    }
}
...
//   uint64_t baseOccurrence = index->bwtBlockList.asNucleotide[blockIndex].baseOccurrences[letter];
//   __m256i occurrenceVector =
//   awFmMakeNucleotideOccurrenceVector(&(index->bwtBlockList.asNucleotide[blockIndex]),
//     letter);
//
//   uint_fast8_t vectorPopcount = awFmVectorPopcount(occurrenceVector, localQueryPosition);
//
...
//   uint8_t *newStartBlockPtr = ((uint8_t*)index->bwtBlockList.asNucleotide) + (newStartBlock *
//   sizeof(struct AwFmNucleotideBlock)); _mm_prefetch(newStartBlockPtr, AW_FM_PREFETCH_STRATEGY);
//   _mm_prefetch(newStartBlockPtr + 64, AW_FM_PREFETCH_STRATEGY);
//
//   return result;
// }
...
//   uint64_t baseOccurrence   = index->bwtBlockList.asAmino[blockIndex].baseOccurrences[letter];
//   __m256i occurrenceVector  =
//   awFmMakeAminoAcidOccurrenceVector(&(index->bwtBlockList.asAmino[blockIndex]),
//     letter);
//   uint_fast8_t  vectorPopcount  = awFmVectorPopcount(occurrenceVector, localQueryPosition);
//
...
//   //prefetch the next start ptr
//   uint64_t newStartBlock    = (result - 1) / AW_FM_POSITIONS_PER_FM_BLOCK;
//   uint8_t *newStartBlockPtr = ((uint8_t*)index->bwtBlockList.asAmino) + (newStartBlock *
//   sizeof(struct AwFmAminoBlock)); for(size_t cacheLine = 0; cacheLine < 5; cacheLine++){
//     _mm_prefetch(newStartBlockPtr + (cacheLine * 64), AW_FM_PREFETCH_STRATEGY);
//   }
//
//   return result;
