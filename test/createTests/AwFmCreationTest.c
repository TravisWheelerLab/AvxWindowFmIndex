...
const uint8_t aminoLookup[21] = {'a', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'k', 'l', 'm',
                                 'n', 'p', 'q', 'r', 's', 't', 'v', 'w', 'y', 'z'};
...
void testKmerTableLengths(const struct AwFmIndex *restrict const index, const uint8_t *sequence,
...
void testPrefixSums(const struct AwFmIndex *restrict const index, const uint8_t *sequence,
                    const size_t sequenceLength);
...
struct AwFmIndex *testCreateNucleotideIndex(const uint8_t *sequence, const size_t sequenceLength) {
    ...
    enum AwFmReturnCode returnCode =
        awFmCreateIndex(&index, &metadata, sequence, sequenceLength, fileSrc, true);
    sprintf(buffer, "return code was not successful, returned %d", returnCode);
    testAssertString(returnCode >= 0, buffer);

    sprintf(buffer, "version number expected %d, got %d", metadata.versionNumber,
            index->metadata.versionNumber);
    ...
    sprintf(buffer, "suffix array compression ratio expected %d, got %d",
            metadata.suffixArrayCompressionRatio, index->metadata.suffixArrayCompressionRatio);
    testAssertString(metadata.suffixArrayCompressionRatio ==
                         index->metadata.suffixArrayCompressionRatio,
                     buffer);

    sprintf(buffer, "kmerLengthInSeedTable expected %d, got %d", metadata.kmerLengthInSeedTable,
            index->metadata.kmerLengthInSeedTable);
    testAssertString(metadata.kmerLengthInSeedTable == index->metadata.kmerLengthInSeedTable,
                     buffer);

    sprintf(buffer, "alphabetType expected %d, got %d", metadata.alphabetType,
            index->metadata.alphabetType);
    ...
    sprintf(buffer, "in memory suffix array flag should be (%u), but was %u",
            keepSuffixArrayInMemory, index->metadata.keepSuffixArrayInMemory);
    testAssertString(index->metadata.keepSuffixArrayInMemory == keepSuffixArrayInMemory, buffer);

    sprintf(buffer, "bwt length should be equal to sequence length + 1 (%zu), got %zu",
...
void testKmerTableLengths(const struct AwFmIndex *restrict const index, const uint8_t *sequence,
    ...
    for (size_t sequencePosition = 0; sequencePosition <= sequenceLength - kmerLength;
         sequencePosition++) {
        ...
        sprintf(buffer,
                "for kmer %.*s, found %zu occurrences, but table stored %zu. (range %zu-%zu)\n",
...
void testCreateAminoIndex(void) {
    ...
    struct AwFmIndexMetadata metadata = {.versionNumber = 1,
                                         .suffixArrayCompressionRatio = 100,
    ...
    for (size_t i = 0; i < sequenceLength + 1; i++) {
        ...
        sprintf(buffer,
                "sequence @ i=%zu holds character encoding %#04x (letter %c), but bwt held "
                "encoding %#04x",
