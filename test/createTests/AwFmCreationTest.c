...
struct AwFmIndex *testCreateNucleotideIndex(const uint8_t *sequence, const size_t sequenceLength) {
	...
	sprintf(buffer, "suffix array compression ratio expected %d, got %d", config.suffixArrayCompressionRatio,
	...
	sprintf(buffer, "kmerLengthInSeedTable expected %d, got %d", config.kmerLengthInSeedTable,
	...
	sprintf(buffer, "in memory suffix array flag should be (%u), but was %u", keepSuffixArrayInMemory,
			index->config.keepSuffixArrayInMemory);
	testAssertString(index->config.keepSuffixArrayInMemory == keepSuffixArrayInMemory, buffer);

	sprintf(
			buffer, "bwt length should be equal to sequence length + 1 (%zu), got %zu", sequenceLength + 1, index->bwtLength);
...
void testKmerTableLengths(
	...
	for(size_t sequencePosition = 0; sequencePosition <= sequenceLength - kmerLength; sequencePosition++) {
		...
		sprintf(buffer, "for kmer %.*s, found %zu occurrences, but table stored %zu. (range %zu-%zu)\n", kmerLength,
				kmerPtr1, kmerCount, numInRange, rangeInTable.startPtr, rangeInTable.endPtr);
...
void testCreateAminoIndex(void) {
	...
	for(size_t i = 0; i < sequenceLength + 1; i++) {
		...
		uint8_t encodedLetterFromSequence =
				(suffixArray[i] == 0) ?
						0 :
						awFmAminoAcidLetterIndexToCompressedVector(awFmAsciiAminoAcidToLetterIndex(letterFromSequence));
		...
		sprintf(buffer, "sequence @ i=%zu holds character encoding %#04x (letter %c), but bwt held encoding %#04x", i,
				encodedLetterFromSequence, letterFromSequence, letterFromBwt);
