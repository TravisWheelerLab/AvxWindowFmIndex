	...
	for(size_t testNum = 0; testNum < 100; testNum++) {
		...
		for(size_t sequencePosition = 0; sequencePosition < sequenceLength; sequencePosition++) {
			...
			enum AwFmReturnCode returnCode =
					awFmReadSequenceFromFile(index, sequencePosition, sequenceSegmentLength, sequenceBuffer);

			sprintf(buffer,
					"awFmReadSequenceFromFile returned failure code %i for position %zu (segment length %zu, total len %zu)",
					returnCode, sequencePosition, sequenceSegmentLength, sequenceLength);
			...
			sprintf(buffer,
					"sequence segment %.*s  length %zu at position %zu did not match what was returned in the buffer %.*s",
					(int)sequenceSegmentLength, (char *)(sequence + sequencePosition), sequenceSegmentLength, sequencePosition,
		...
		for(size_t sequencePosition = 0; sequencePosition < sequenceLength; sequencePosition++) {
			...
			sprintf(buffer, "sequence segment %.*s did not match what was returned in the buffer %.*s",
					(int)sequenceSegmentLength, (char *)(sequence + sequencePosition), (int)sequenceSegmentLength,
					sequenceBuffer);
...
void suffixArrayTest(void) {
	...
	for(size_t compressionRatio = 1; compressionRatio < 32; compressionRatio++) {
		...
		int64_t *referenceSuffixArray = malloc((sequenceLength + 1) * sizeof(uint64_t));
		divsufsort64(sequence, (int64_t *)(referenceSuffixArray + 1), sequenceLength);
		...
		for(size_t i = 0; i < numElementsInSuffixArray; i++) {
			...
			sprintf(buffer, "on position %zu, backtrace position %zu did not match value in suffix array (%zu).", i,
					backtrace.position, positionFromSuffixArray);
...
void indexReadTest(void) {
	...
	for(size_t testNum = 0; testNum < 100; testNum++) {
		...
		// version number
		sprintf(buffer, "the version numbers of the original (%i) and the one from file (%i) did not match.",
				index->versionNumber, indexFromFile->versionNumber);
		testAssertString(index->versionNumber == indexFromFile->versionNumber, buffer);

		// suffix array compression ratio
		sprintf(buffer,
				"the suffix array compression ratios of the original (%i) and the one from file (%i) did not match.",
				index->config.suffixArrayCompressionRatio, indexFromFile->config.suffixArrayCompressionRatio);
		testAssertString(
				index->config.suffixArrayCompressionRatio == indexFromFile->config.suffixArrayCompressionRatio, buffer);

		// kmer length in seed table
		sprintf(buffer,
				"the kmer length in seed table values of the original (%i) and the one from file (%i) did not match.",
				index->config.kmerLengthInSeedTable, indexFromFile->config.kmerLengthInSeedTable);
		testAssertString(index->config.kmerLengthInSeedTable == indexFromFile->config.kmerLengthInSeedTable, buffer);

		// alphabet type
		sprintf(buffer, "the alphabet type of the original (%i) and the one from file (%i) did not match.",
				index->config.alphabetType, indexFromFile->config.alphabetType);
		testAssertString(index->config.alphabetType == indexFromFile->config.alphabetType, buffer);

		// suffix array file offset
		sprintf(buffer, "the suffixArrayFileOffset of the original (%zu) and the one from file (%zu) did not match.",
				index->suffixArrayFileOffset, indexFromFile->suffixArrayFileOffset);
		testAssertString(index->suffixArrayFileOffset == indexFromFile->suffixArrayFileOffset, buffer);

		// sequenceFileOffset
		sprintf(buffer, "the sequenceFileOffset of the original (%zu) and the one from file (%zu) did not match.",
		...
		testAssertString(
				memcmp(index->bwtBlockList.asNucleotide, indexFromFile->bwtBlockList.asNucleotide, blockListLengthInBytes) == 0,
				buffer);
		...
		// kmerSeedTable
		size_t kmerTableLengthInBytes = awFmGetKmerTableLength(index) * sizeof(struct AwFmSearchRange);

		sprintf(buffer, "the kmer table of the original and the one from file did not match.");
		testAssertString(memcmp(index->kmerSeedTable, indexFromFile->kmerSeedTable, kmerTableLengthInBytes) == 0, buffer);

		// dealloc both indices
		// here, we're cheating by doing what dealloc does, except not trying to close the already closed file
