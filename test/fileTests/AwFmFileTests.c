...
char buffer[2048];
uint8_t aminoLookup[21] = {'$', 'a', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'k', 'l',
                           'm', 'n', 'p', 'q', 'r', 's', 't', 'v', 'w', 'y'};
...
void sequenceRecallTest(void) {
    ...
    for (size_t testNum = 0; testNum < 100; testNum++) {
        ...
        struct AwFmIndexMetadata metadata = {.versionNumber = 1,
                                             .suffixArrayCompressionRatio = 240,
        ...
        for (size_t sequencePosition = 0; sequencePosition < sequenceLength; sequencePosition++) {
            ...
            enum AwFmReturnCode returnCode = awFmReadSequenceFromFile(
                index, sequencePosition, priorFlankLength, postFlankLength, sequenceBuffer);

            sprintf(buffer, "awFmReadSequenceFromFile returned failure code %i", returnCode);
            testAssertString(returnCode > 0, buffer);

            size_t numPriorCharacters =
                sequencePosition > priorFlankLength ? priorFlankLength : sequencePosition;
            size_t numPostCharacters = (sequencePosition + postFlankLength) > (index->bwtLength - 1)
                                           ? (index->bwtLength - 1) - sequencePosition
                                           : postFlankLength;
            int sequenceSegmentLength = numPriorCharacters + numPostCharacters;
            size_t sequenceSegmentStartPosition =
                priorFlankLength == numPriorCharacters ? sequencePosition - numPriorCharacters : 0;

            int compareResult =
                strncmp(sequenceBuffer, (char *)(sequence + sequenceSegmentStartPosition),
                        sequenceSegmentLength);
            if (compareResult != 0) {
                printf("seq seg len: %i, res %i, seq: %.*s, buf %.*s, first l %u, %u, return code "
                       "%u\n",
                       sequenceSegmentLength, compareResult, sequenceSegmentLength,
                       (char *)(sequence + sequenceSegmentStartPosition), sequenceSegmentLength,
                       sequenceBuffer, sequence[sequenceSegmentStartPosition], sequenceBuffer[0],
                       returnCode);
                printf("seq pos: %zu, prior %zu, post %zu\n", sequencePosition, numPriorCharacters,
                       numPostCharacters);
            }
            sprintf(buffer,
                    "sequence segment %.*s did not match what was returned in the buffer %.*s",
                    sequenceSegmentLength, (char *)(sequence + sequenceSegmentStartPosition),
...
void suffixArrayTest(void) {
    ...
    for (size_t compressionRatio = 1; compressionRatio < 32; compressionRatio++) {
        ...
        struct AwFmIndex *index;
        struct AwFmIndexMetadata metadata = {.versionNumber = 1,
                                             .suffixArrayCompressionRatio = compressionRatio,
                                             .kmerLengthInSeedTable = 2,
                                             .alphabetType = AwFmAlphabetNucleotide,
                                             .keepSuffixArrayInMemory = false};
        ...
        for (size_t i = 0; i < sequenceLength + 1; i += compressionRatio) {
            ...
            sprintf(buffer,
                    "on position %zu, suffix array read position parallel returned error code %i.",
                    i, returnCode);
            testAssertString(returnCode > 0, buffer);

            sprintf(buffer,
                    "on position %zu, backtrace position %zu did not match value in suffix array "
                    "(%zx).",
...
void indexReadTest(void) {
    ...
    for (size_t testNum = 0; testNum < 100; testNum++) {
        ...
        const enum AwFmAlphabetType alphabetType =
            rand() & 1 ? AwFmAlphabetNucleotide : AwFmAlphabetAmino;
        ...
        for (size_t i = 0; i < sequenceLength; i++) {
            sequence[i] = alphabetType == AwFmAlphabetNucleotide ? nucleotideLookup[rand() % 5]
                                                                 : aminoLookup[rand() % 21];
        }

        struct AwFmIndex *index;
        struct AwFmIndexMetadata metadata = {.versionNumber = 1,
                                             .suffixArrayCompressionRatio = 255,
        ...
        returnCode = awFmReadIndexFromFile(&indexFromFile, "testindex.awfmi", false);
        if (returnCode < 0) {
        ...
        sprintf(buffer,
                "the bwt lengths of the original (%zu) and the one from file (%zu) did not match.",
        ...
        sprintf(
            buffer,
            "the version numbers of the original (%i) and the one from file (%i) did not match.",
            index->metadata.versionNumber, indexFromFile->metadata.versionNumber);
        testAssertString(index->metadata.versionNumber == indexFromFile->metadata.versionNumber,
                         buffer);

        // suffix array compression ratio
        sprintf(buffer,
                "the suffix array compression ratios of the original (%i) and the one from file "
                "(%i) did not match.",
                index->metadata.suffixArrayCompressionRatio,
                indexFromFile->metadata.suffixArrayCompressionRatio);
        testAssertString(index->metadata.suffixArrayCompressionRatio ==
                             indexFromFile->metadata.suffixArrayCompressionRatio,
                         buffer);

        // kmer length in seed table
        sprintf(buffer,
                "the kmer length in seed table values of the original (%i) and the one from file "
                "(%i) did not match.",
                index->metadata.kmerLengthInSeedTable,
                indexFromFile->metadata.kmerLengthInSeedTable);
        testAssertString(index->metadata.kmerLengthInSeedTable ==
                             indexFromFile->metadata.kmerLengthInSeedTable,
                         buffer);

        // alphabet type
        sprintf(buffer,
                "the alphabet type of the original (%i) and the one from file (%i) did not match.",
                index->metadata.alphabetType, indexFromFile->metadata.alphabetType);
        testAssertString(index->metadata.alphabetType == indexFromFile->metadata.alphabetType,
                         buffer);

        // suffix array file offset
        sprintf(buffer,
                "the suffixArrayFileOffset of the original (%zu) and the one from file (%zu) did "
                "not match.",
                index->suffixArrayFileOffset, indexFromFile->suffixArrayFileOffset);
        testAssertString(index->suffixArrayFileOffset == indexFromFile->suffixArrayFileOffset,
                         buffer);

        // sequenceFileOffset
        sprintf(buffer,
                "the sequenceFileOffset of the original (%zu) and the one from file (%zu) did not "
                "match.",
                index->sequenceFileOffset, indexFromFile->sequenceFileOffset);
        testAssertString(index->sequenceFileOffset == indexFromFile->sequenceFileOffset, buffer);
        ...
        // prefix sums
        sprintf(buffer, "the prefix sums of the original and the one from file did not match.");
        testAssertString(memcmp(index->prefixSums, indexFromFile->prefixSums,
        ...
        sprintf(buffer, "the kmer table of the original and the one from file did not match.");
        testAssertString(
            memcmp(index->kmerSeedTable, indexFromFile->kmerSeedTable, kmerTableLengthInBytes) == 0,
            buffer);

        // dealloc both indices
        // here, we're cheating by doing what dealloc does, except not trying to close the already
        // closed file
