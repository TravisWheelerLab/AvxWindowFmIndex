...
void inMemorySaUncompressedTest(void) {
    for (size_t testNum = 0; testNum < 500; testNum++) {
        ...
        for (size_t kmerIndex = 0; kmerIndex < numKmers; kmerIndex++) {
            struct AwFmBacktrace *backtraceList =
            ...
            for (size_t sequencePosition = 0; sequencePosition < sequenceLength;
                ...
                sprintf(buffer,
                        ...
                        kmerLength, searchList->kmerSearchData[kmerIndex].kmerString,
                        kmerFoundAtPosition, positionInBacktraceList);
                if (kmerFoundAtPosition != positionInBacktraceList) {
                    ...
                    for (size_t i = 0; i < searchList->kmerSearchData[kmerIndex].count; i++) {
                        printf("[%zu, (%zu)], ", backtraceList[i].position,
...
void inMemorySaCompressedTest(void) {
    for (size_t testNum = 0; testNum < 500; testNum++) {
        ...
        struct AwFmIndexMetadata metadata = {.versionNumber = 1,
                                             .suffixArrayCompressionRatio = sequenceLength - 1,
                                             .kmerLengthInSeedTable = 8,
                                             .alphabetType = AwFmAlphabetNucleotide,
                                             .keepSuffixArrayInMemory = true};
...
void inMemoryFromFileTest(void) {
    for (size_t testNum = 0; testNum < 500; testNum++) {
        ...
        struct AwFmIndex *index;
        struct AwFmIndexMetadata metadata = {.versionNumber = 1,
                                             .suffixArrayCompressionRatio = 16,
                                             .kmerLengthInSeedTable = 8,
                                             .alphabetType = AwFmAlphabetNucleotide,
                                             .keepSuffixArrayInMemory = true};
