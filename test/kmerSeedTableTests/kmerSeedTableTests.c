...
void checkRangeForCorrectness(const struct AwFmSearchRange *restrict const range,
                              const uint64_t *restrict const suffixArray, const char *kmer,
                              const uint8_t kmerLength, const uint8_t *restrict const sequence,
...
void checkRangeForCorrectness(const struct AwFmSearchRange *restrict const range,
    ...
    for (uint64_t position = 0; position <= sequenceLength - kmerLength; position++) {
        ...
        // printf("kmer %.*s at pos %zu, matches kmer %.*s\n", kmerLength, sequence+position,
        // position, kmerLength, kmer);
        bool positionFoundInRange = false;

        for (uint64_t suffixArrayPosition = range->startPtr; suffixArrayPosition <= range->endPtr;
             suffixArrayPosition++) {
        ...
        if (!positionFoundInRange) {
            ...
            sprintf(buffer,
                    "range had position %zu for kmer %s, but that position holds kmer %s.\n",
...
void testAllKmerRanges(const struct AwFmIndexMetadata *metadata, uint64_t sequenceLength) {
    ...
    for (uint8_t testNum = 0; testNum < 4; testNum++) {
        ...
        for (uint64_t i = 0; i < sequenceLength; i++) {
            sequence[i] =
                (metadata->alphabetType == AwFmAlphabetNucleotide ? nucleotideLookup[rand() % 5]
                                                                  : aminoLookup[rand() % 21]);
        }
        ...
        for (size_t kmerIndex = 0; kmerIndex < tableLength; kmerIndex++) {
            ...
            checkRangeForCorrectness(&kmerRange, suffixArray, kmer, kmerLength, sequence,
                                     sequenceLength);
        }
