...
void checkArgs() {
    ...
    if (numQueries == 0) {
        printf("Num queries was not given (or was given 0). please provide the number of queries "
               "to perform with the -q flag\n");
        allArgumentsParsed = false;
    }
    if (dbSizeInWindows == 0) {
        printf("database size was not given (or was given 0). please provide the size of the db to "
               "test with the -s flag\n");
        allArgumentsParsed = false;
    }
