...
void popcountTestSuite(void) {
    ...
    sprintf(buffer, "vector popcount of all zeros should be all ones (256), returned %d.",
            popcount);
    ...
    for (uint16_t testNum = 0; testNum < numRandTests; testNum++) {
        ...
        vector = _mm256_loadu_si256((__m256i *)vectorBytes);
        popcount = awFmVectorPopcount(vector, 255);
        sprintf(buffer, "vector popcount of all zeros should be %d, returned %d.", bitsSet,
                popcount);
