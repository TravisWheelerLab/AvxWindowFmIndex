...
void testParallelSearchAmino(void) {
    ...
    for (uint8_t numThreads = 1; numThreads < 16; numThreads++) {
        ...
        for (size_t kmerIndex = 0; kmerIndex < searchList->count; kmerIndex++) {
            ...
            //   positionList->backtraceArray[i].position);
            // }
            ...
            for (size_t sequencePosition = 0; sequencePosition < sequenceLength;
                ...
                bool kmerFoundAtThisPosition =
                    (strncmp((char *)&sequence[sequencePosition], searchData->kmerString,
                             searchData->kmerLength)) == 0;
                ...
                if (kmerFoundAtThisPosition && !thisPositionInBacktraceList) {
                    ...
                    for (size_t i = 0; i < searchData->count; i++) {
                        printf("pos: %zu, off %zu, ", backtraceList[i].position,
                               backtraceList[i]._offset);
                    }
                    printf("\n");
                    printf("debug: ");
                    ...
                    sprintf(buffer,
                            "kmer %.*s (%.*s) was found at position %zu, but was not represented "
                            "in the backtrace vector.",
                    ...
                    printf("(seq pos = %zu), pos list count %u\n", sequencePosition,
                           searchData->count);
                    for (size_t i = 0; i < searchData->count; i++) {
                        printf("pos: [%zu, offset %zu], ",
                               searchData->positionBacktraceList[i].position,
                               searchData->positionBacktraceList[i]._offset);
                    }
                    printf("\n");
                    ...
                    printf("\n");
                    sprintf(buffer,
                            "kmer %.*s was in the backtrace vector, but was not found at the "
                            "expected sequence position %zu (%.*s found there instead).",
                            (uint32_t)searchData->kmerLength, searchData->kmerString,
                            sequencePosition, (uint32_t)searchData->kmerLength,
                            &sequence[sequencePosition]);
                    testAssertString(false, buffer);
    ...
    for (uint8_t numThreads = 1; numThreads < 16; numThreads++) {
        ...
        for (size_t kmerIndex = 0; kmerIndex < searchList->count; kmerIndex++) {
            ...
            for (size_t sequencePosition = 0;
                 sequencePosition < sequenceLength - searchData->kmerLength; sequencePosition++) {
                bool kmerFoundAtThisPosition =
                    (strncmp((char *)&sequence[sequencePosition], searchData->kmerString,
                             searchData->kmerLength)) == 0;
                officialKmerCount += kmerFoundAtThisPosition ? 1 : 0;
            }
            sprintf(buffer, "for kmer index %zu, returned count %u did not match actual count %zu.",
