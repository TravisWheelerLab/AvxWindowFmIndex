...
void testParallelSearchAmino(void) {
	...
	for(size_t saIndex = 0; saIndex < sequenceLength + 1; saIndex += saCompressionRatio) {
		...
		sprintf(buffer, "index compressed SA value @ index %zu (%zu) did not match reference val %zu.", saIndex, position,
				refValue);
		...
		for(size_t kmerIndex = 0; kmerIndex < searchList->count; kmerIndex++) {
			...
			for(size_t sequencePosition = 0; sequencePosition < sequenceLength; sequencePosition++) {
				bool kmerFoundAtThisPosition =
						(strncmp((char *)&sequence[sequencePosition], searchData->kmerString, searchData->kmerLength)) == 0;
				...
				if(kmerFoundAtThisPosition && !thisPositionInList) {
					...
					printf("\n");
					sprintf(buffer,
							"kmer %.*s was in the position list, but was not found at the expected sequence position %zu (%.*s found "
							"there instead).",
							(uint32_t)searchData->kmerLength, searchData->kmerString, sequencePosition,
...
void testParallelCount(void) {
	...
	for(uint8_t numThreads = 1; numThreads < 16; numThreads++) {
		...
		for(size_t kmerIndex = 0; kmerIndex < searchList->count; kmerIndex++) {
			...
			for(size_t sequencePosition = 0; sequencePosition < sequenceLength - searchData->kmerLength; sequencePosition++) {
				bool kmerFoundAtThisPosition =
						(strncmp((char *)&sequence[sequencePosition], searchData->kmerString, searchData->kmerLength)) == 0;
				officialKmerCount += kmerFoundAtThisPosition ? 1 : 0;
			}
			sprintf(buffer, "for kmer index %zu, returned count %u did not match actual count %zu.", kmerIndex,
					searchData->count, officialKmerCount);
