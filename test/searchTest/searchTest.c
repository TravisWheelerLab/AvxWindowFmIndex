...
void testSearchForRandomKmers(
		const struct AwFmIndex *index, uint64_t numKmers, uint8_t *sequence, size_t sequenceLength, uint64_t *suffixArray);
bool sequencePositionInRange(
		const struct AwFmSearchRange *searchRange, const uint64_t position, const uint64_t *suffixArray);
...
void generateRandomIndex(struct AwFmIndex **index, uint8_t **sequence, size_t sequenceLength, uint64_t **suffixArray) {
	...
	*sequence =
			realloc((*sequence), (sequenceLength + 100) * sizeof(uint8_t));	 //+11 is added for padding when using strcmp
...
void testRangeForCorrectness(const struct AwFmSearchRange *range, const uint8_t *sequence, const size_t sequenceLength,
	...
	for(size_t position = 0; position < sequenceLength; position++) {
		...
		if(kmerFoundAtPosition && !kmerExpectedAtPosition) {
			sprintf(buffer,
					"kmer  \"%.*s\" was found at position %zu, (%.*s) but was not expected in range [%zu, %zu]:", kmerLength,
					kmer, position, kmerLength, sequence + position, range->startPtr, range->endPtr);
