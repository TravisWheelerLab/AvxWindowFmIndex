...
char buffer[2048];
uint8_t aminoLookup[21] = {'a', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'k', 'l', 'm',
                           'n', 'p', 'q', 'r', 's', 't', 'v', 'w', 'y', 'z'};
uint8_t nucleotideLookup[5] = {'a', 'c', 'g', 't', 'x'};

void generateRandomIndex(struct AwFmIndex **index, uint8_t **sequence, size_t sequenceLength,
                         uint64_t **suffixArray);
void testSearchForRandomKmers(const struct AwFmIndex *index, uint64_t numKmers, uint8_t *sequence,
...
void testRangeForCorrectness(const struct AwFmSearchRange *range, const uint8_t *sequence,
                             const size_t sequenceLength, const uint64_t *suffixArray,
                             const char *kmer, const uint8_t kmerLength);
...
void generateRandomIndex(struct AwFmIndex **index, uint8_t **sequence, size_t sequenceLength,
    ...
    struct AwFmIndexMetadata metadata = {.versionNumber = 1,
                                         .suffixArrayCompressionRatio = 200,
    ...
    *sequence =
        realloc((*sequence), (sequenceLength + 100) *
    ...
    *suffixArray = realloc((*suffixArray), (sequenceLength + 1) * sizeof(uint64_t));
    ...
    uint8_t *characterLookupTable =
        alphabetType == AwFmAlphabetNucleotide ? nucleotideLookup : aminoLookup;
...
void testSearchForRandomKmers(const struct AwFmIndex *index, uint64_t numKmers, uint8_t *sequence,
    ...
    for (size_t kmerNum = 0; kmerNum < numKmers; kmerNum++) {
        ...
        // build the kmer.
        // printf("nuclookup ptr %p, amino %p, index p: %p\n", nucleotideLookup, aminoLookup,
        // index);
        uint8_t *characterLookupTable = (index->metadata.alphabetType == AwFmAlphabetNucleotide)
                                            ? &nucleotideLookup[0]
                                            : &aminoLookup[0];
...
void testRangeForCorrectness(const struct AwFmSearchRange *range, const uint8_t *sequence,
    ...
    for (size_t position = 0; position < sequenceLength; position++) {
        ...
        if (kmerFoundAtPosition && !kmerExpectedAtPosition) {
            sprintf(buffer,
                    "kmer  \"%.*s\" was found at position %zu, (%.*s) but was not expected in "
                    "range [%zu, %zu]:",
                    kmerLength, kmer, position, kmerLength, sequence + position, range->startPtr,
                    range->endPtr);
            ...
            sprintf(buffer,
                    "kmer  \"%.*s\" returned range [%zu, %zu], which contains position %zu, but "
                    "that position was not in range:",
