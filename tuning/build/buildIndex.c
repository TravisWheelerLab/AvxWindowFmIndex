...
void buildChromosomeIndex(int chromosomeNumber, uint8_t suffixArrayCompressionRatio,
                          uint8_t kmerLengthInSeedTable, char *indexFilename);
void buildFullGenomeIndex(uint8_t suffixArrayCompressionRatio, uint8_t kmerLengthInSeedTable,
                          char *indexFilename);
...
int main(int argc, char **argv) {
    ...
    printf("parse finished: isAmino %i, chrNo %i, fulGenome %i, sacr %i, klist %i\n",
           isAminoSequence, chromosomeNumber, fullGenomeIndexRequested, suffixArrayCompressionRatio,
    ...
    if (chromosomeNumber == -1 && !fullGenomeIndexRequested) {
        ...
        printf("the full genome flag was set, but a specific chromosome was also requested. "
               "Ignoring full genome and making index on chromosome %i\n",
               chromosomeNumber);
        fullGenomeIndexRequested = false;
    }
    ...
    if (fullGenomeIndexRequested) {
        buildFullGenomeIndex(suffixArrayCompressionRatio, kmerLengthInSeedTable,
                             indexFilenameBuffer);
    } else {
        buildChromosomeIndex(chromosomeNumber, suffixArrayCompressionRatio, kmerLengthInSeedTable,
                             indexFilenameBuffer);
    }
}
...
void buildChromosomeIndex(int chromosomeNumber, uint8_t suffixArrayCompressionRatio,
    ...
    int64_t sequenceLength =
        getChromosomeFromFullFasta(fullGenomeFile, chromosomeNumber, sequenceBuffer);
    if (sequenceLength == 0) {
    ...
    if (sequenceLength > 1000) {
        ...
        printf("for reference, here's all %zu characters in the sequence: %.*s\n", sequenceLength,
               (int)sequenceLength, sequenceBuffer);
    }

    enum AwFmReturnCode returnCode = awFmCreateIndex(&index, &metadata, (uint8_t *)sequenceBuffer,
                                                     sequenceLength, indexFilename, true);
    ...
    while (!feof(sequenceFile)) {
        ...
        if (isHeader) {
            bool thisHeaderIsCorrectChromosomeHeader =
                (chromosomeNumber < 10 &&
                 buffer[sequenceLength + 4] == chromosomeNumberOnesPlace) ||
                (chromosomeNumber >= 10 &&
                 buffer[sequenceLength + 4] == chromosomeNumberTensPlace &&
                 buffer[sequenceLength + 5] == chromosomeNumberOnesPlace);
            ...
            // if we just were reading the correct chromosome, and just hit a header for a new
            // chromosome, we're done.
        ...
        // if we're reading from the correct chromosome, move the buffer offset to preserve the read
        // data.
