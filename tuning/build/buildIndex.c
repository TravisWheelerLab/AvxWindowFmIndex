...
void buildChromosomeIndex(
		int chromosomeNumber, uint8_t suffixArrayCompressionRatio, uint8_t kmerLengthInSeedTable, char *indexFilename);
...
int main(int argc, char **argv) {
	...
	printf("parse finished: isAmino %i, chrNo %i, fulGenome %i, sacr %i, klist %i\n", isAminoSequence, chromosomeNumber,
			fullGenomeIndexRequested, suffixArrayCompressionRatio, kmerLengthInSeedTable);
...
void parseArgs(int argc, char **argv) {
	...
	while((option = getopt(argc, argv, "ac:gs:k:f:")) != -1) {
		switch(option) {
			case 'a': isAminoSequence = true; break;
			case 'c':
				sscanf(optarg, "%i", &chromosomeNumber);
				printf("chrom # set to %i in parseArgs\n", chromosomeNumber);
				chromosomeNumberSetInArgs = true;
				break;
			case 'g': fullGenomeIndexRequested = true; break;
			case 's': sscanf(optarg, "%i", &suffixArrayCompressionRatio); break;
			case 'k': sscanf(optarg, "%i", &kmerLengthInSeedTable); kmerLengthSetInArgs = true;
...
void buildChromosomeIndex(
	...
	enum AwFmReturnCode returnCode =
			awFmCreateIndex(&index, &metadata, (uint8_t *)sequenceBuffer, sequenceLength, indexFilename, true);
	...
	while(!feof(sequenceFile)) {
		...
		if(isHeader) {
			bool thisHeaderIsCorrectChromosomeHeader =
					...
					(chromosomeNumber >= 10 && buffer[sequenceLength + 4] == chromosomeNumberTensPlace &&
