...
void fillSearchListwithKmers(struct AwFmKmerSearchList *searchList, size_t numKmers, uint8_t kmerLength, struct AwFmIndex *index);
bool charIsAmbiguous(char c, enum AwFmAlphabetType alphabet);
bool getStringFromSequenceFile(FILE *openSequenceFile, size_t position, char *buffer, int kmerLength, enum AwFmAlphabetType alphabet);
void makeValidQueryFromSequenceFile(FILE *openSequenceFile, size_t sequenceLength,
   uint8_t kmerLength, enum AwFmAlphabetType alphabet, char *queryBuffer);
  ...
  while((option = getopt(argc, argv, "mcf:j:n:k:t:")) != -1){
    ...
    switch(option){
      ...
      case 'f':
        ...
        break;
      case 'j':
        strcpy(sequenceFilenameBuffer, optarg);
        break;
      case 'n':
...
void fillSearchListwithKmers(struct AwFmKmerSearchList *searchList, size_t numKmers, uint8_t kmerLength, struct AwFmIndex *index){
  ...
  for(size_t i = 0; i < numKmers;i++){
    ...
    makeValidQueryFromSequenceFile(sequenceFile, index->bwtLength - 1, kmerLength,
      index->metadata.alphabetType, searchList->kmerSearchData[i].kmerString);
  }
}
